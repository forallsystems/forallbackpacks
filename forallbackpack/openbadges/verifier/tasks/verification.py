...
def verify_hosted_assertion_not_revoked(state, task_meta, **options):
    try:
        node_id = task_meta['node_id']        
        assertion_node = [n for n in state['graph'] if n.get('id') == node_id][0]

        if assertion_node['revoked']:
            return task_result(False, assertion_node.get('revocationReason', ''))
    except (IndexError, KeyError):
        raise TaskPrerequisitesError()
    
    return task_result(True, "Assertion {} has not been revoked".format('TESTING'))