...
from .task_types import (DETECT_AND_VALIDATE_NODE_CLASS, FETCH_HTTP_NODE, INTAKE_JSON, JSONLD_COMPACT_DATA,
                         ...
                         VERIFY_HOSTED_ASSERTION_NOT_REVOKED, ASSERTION_VALIDATED)
...
def fetch_http_node(state, task_meta, **options):
    ...
    if result.status_code == 410 and expected_class == OBClasses.Assertion:
        expected_class = OBClasses.RevokedAssertion
    ...
    if expected_class == OBClasses.RevokedAssertion:
        actions += [add_task(VERIFY_HOSTED_ASSERTION_NOT_REVOKED, node_id=url,
        prerequisites=ASSERTION_VALIDATED)]
...
# NEW CODE -->
    if isinstance(value, list):
        ...
        return task_result(
            False, "Property {} referenced from {} is not a list, JSON object or string as expected".format(
                prop_name, abv_node(node_id=node_id)
            ))
    ...
    for value in value_list:
        ...
        if not isinstance(value, dict):
            return task_result(
                False, "Property {} item referenced from {} is not a JSON object or string as expected".format(
                    prop_name, abv_node(node_id=node_id)
                ))
